  ...
  return (
    <div className="card flex flex-col gap-5 !p-5">
          ...
          {HOUSEHOLD.map((m) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 rounded-full border border-[var(--color-line)] py-1 pl-1 pr-4 text-[14px] font-semibold text-[var(--color-ink)] transition-colors ${
                  selected ? 'bg-[var(--color-accent)] shadow-[var(--shadow-lift)]' : 'bg-[var(--color-surface)]'
                } ${m.voiceReady ? 'cursor-pointer' : 'cursor-not-allowed opacity-45'}`}
              >
                <span
                  className={`grid h-7 w-7 place-items-center rounded-full border border-[var(--color-line)] text-[12px] font-bold ${
                    selected ? 'bg-white' : 'bg-[var(--color-panel-2)]'
                  }`}
                >
                  {m.name[0]}
                </span>
      ...
      <section
        ...
        className="flex min-h-[220px] max-h-[46vh] flex-col gap-3 overflow-y-auto rounded-[20px] border border-[var(--color-line)] bg-[var(--color-panel)] p-4"
      >
        {messages.length === 0 && (
          ...
          <div key={m.id} className={`flex flex-col ${m.side === 'out' ? 'items-end' : 'items-start'}`}>
            <div
              className={`max-w-[85%] px-4 py-2.5 text-[14px] leading-[1.45] ${
                m.side === 'out'
                  ? 'rounded-[18px] rounded-br-[6px] border border-[var(--color-line)] bg-[var(--color-accent)] font-bold text-[var(--color-ink)]'
                  : 'rounded-[18px] rounded-bl-[6px] border border-[var(--color-line)] bg-[var(--color-surface)] font-bold text-[var(--color-ink)]'
              }`}
      ...
      {!connected && (
        <p className="rounded-[16px] border-2 border-dashed border-[var(--color-tint)] bg-[var(--color-surface)] px-4 py-3 text-[13px] font-semibold text-[var(--color-ink-2)]">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-[var(--color-line)] bg-[var(--color-accent)] text-[var(--color-ink)] shadow-[var(--shadow-lift)] transition-opacity disabled:opacity-35"
          >
        ...
        <button
          ...
          disabled={!canSend}
          onClick={() => void send('')}
          className="self-start rounded-full border border-[var(--color-line)] bg-[var(--color-surface)] px-3.5 py-1.5 text-[13px] font-semibold text-[var(--color-ink)] disabled:opacity-40"
        >
